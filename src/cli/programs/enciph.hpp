...
namespace dla
{
    ...
    private:
        ...
        template<class I, class S>
        matrix<I> buildMatrix(std::vector<uint8_t> data)
        {
            S x;
            x.setRawData(data);
            return x.getMatrix();
        ...
        template<class I, class S1, class S2, class S3, class S4>
        matrix<I> buildMatrixOfSize(int size, std::vector<uint8_t> data)
        {
            if (size == 8)  return buildMatrix<I, S1>(data);
            if (size == 16) return buildMatrix<I, S2>(data);
            if (size == 24) return buildMatrix<I, S3>(data);
            if (size == 32) return buildMatrix<I, S4>(data);
            return matrix<I>(0, 0);
        }
    };
}